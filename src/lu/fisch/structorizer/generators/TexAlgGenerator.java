	...
	protected void generateCode(Case _case, String _indent)
	{
		if (!_case.isDisabled(true)) {
			...
			if (CASE_MACROS[packageIndex][0] != null) {
				...
				String var = expr;
				if (!Syntax.isIdentifier(expr, false, null)) {
					String semi = optionAppendSemicolon() ? ";" : "";
		...
		if (!_jump.isDisabled(true)) {
			...
			if (lines.count() == 0 || lines.getText().trim().isEmpty())
			{
				addCode(JUMP_MACROS[packageIndex][0]
						.replace("%1", transformText(Syntax.getKeyword("preLeave")))
						.replace("%2", "")
			...
			else
			{
				for(int i = 0; i < lines.count(); i++)
				{
					...
					int index = 0;
					String keyword = Syntax.getKeyword("preLeave");
					if (Jump.isReturn(line)) {
						index = 1;
						keyword = Syntax.getKeyword("preReturn");
					}
					else if (Jump.isExit(line)) {
						index = 2;
						keyword = Syntax.getKeyword("preExit");
					}
					else if (Jump.isThrow(line)) {
						index = 3;
						keyword = Syntax.getKeyword("preThrow");
					}

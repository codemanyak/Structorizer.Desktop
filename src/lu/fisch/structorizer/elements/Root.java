...
//			// END KGU#376 2017-04-20
			...
			boolean _condEof, boolean _condRot, boolean _condMov)
	{
		...
		if (_condEof) {
			TokenList condTokens = new TokenList(_ele.getUnbrokenText().getLongString());
			int posEof = -1;
			int posVar = -1;
			while ((posEof = condTokens.indexOf("fileEOF", posEof+1)) >= 0) {
				if (posEof+3 < condTokens.size() && condTokens.get(posEof+1).equals("(")
